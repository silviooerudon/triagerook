...
export const AI_INSECURE_RULES: AiInsecureRule[] = [
  {
    ...
    mask: true,
    regex:
      /(?:your[_-]?(?:api[_-]?key|secret|token|password|client[_-]?secret)|api[_-]?key[_-]?here|insert[_-]?(?:your[_-]?)?(?:api[_-]?)?key[_-]?here|replace[_-]?(?:this|with[_-]?your[_-]?\w+)|change[-_]?me|<your[_-][a-z0-9_]+>|sk-x{6,}|x{12,}|placeholder[_-]?(?:api[_-]?)?(?:key|secret|token))/i,
  },
  {
    id: "ai-todo-security",
